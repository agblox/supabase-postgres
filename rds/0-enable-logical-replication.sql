...
BEGIN
    ...
    THEN
        RAISE NOTICE USING MESSAGE = 'RDS logical replication must be enabled to proceed. In other words, the RDS database must be a member of an AWS parameter group with rds.logical_replication = 1. Change the parameter "rds.logical_replication" to "1" in the parameter group, save, and restart the database.';
    END IF;
END;
$$;
